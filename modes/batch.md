...
Claude Conductor (claude --chrome --dangerously-skip-permissions)
  ...
  │    └─► claude -p worker → report .md + PDF + tracker-line
  │
  └─ Done: merge tracker-additions → DB via career-ops merge + summary
```
...
4. **For each pending URL**:
   a. Chrome: click on the listing → read JD text from DOM
   ...
   g. Chrome: go back → next listing
5. **Pagination**: If no more listings → click "Next" → repeat
...
| Portal layout change | Conductor reasons about HTML, adapts |
